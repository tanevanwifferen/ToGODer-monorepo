...
import { fetchDocumentCapableModels } from "./OpenRouterModels";
...
/**
 * Map a togoder model identifier to its OpenRouter slug.
 *
 * OpenRouter-routed models already use their OpenRouter slug as the enum
 * value (e.g. `anthropic/claude-sonnet-4`). OpenAI-direct models use a bare
 * name (e.g. `gpt-4o`) which OpenRouter lists under an `openai/` prefix.
 */
export function toOpenRouterSlug(model: AIProvider): string {
  const slug = String(model);
  return slug.includes("/") ? slug : `openai/${slug}`;
}

/**
 * Returns true when the model can accept document/file input (PDF),
 * discovered dynamically from OpenRouter's `input_modalities`. The result is
 * cached (1h TTL); see OpenRouterModels.ts. OpenAI-direct models are looked
 * up via their `openai/`-prefixed OpenRouter slug.
 */
export async function modelSupportsDocuments(
  model: AIProvider,
): Promise<boolean> {
  const capable = await fetchDocumentCapableModels();
  return capable.has(toOpenRouterSlug(model));
}
